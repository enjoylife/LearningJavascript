...
d3.json("smud.json", function(json) {
    var data = [];
    ...
    bills.forEach(function(bill){
        ...
        for(var i = 0, len = bill["flat_charges"].length; i < len; i++) {
            sum += bill["flat_charges"][i]["total"];
        }
        sum = +sum.toFixed(2);
        recorded_years[year].push(sum)
  ...
  console.log(data);
  console.log(recorded_years);
  console.log(d3.values(recorded_years));
  chart.domain([min, max]);

  var vis = d3.select("div").selectAll("svg")
      .data(d3.values(recorded_years))
    .enter().append("svg")
      .attr("class", "box")
